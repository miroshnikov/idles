...
function intersection(...$args)
{
    ...
    return curryN($arity, 
        fn (?iterable $record1, ?iterable $record2) => 
            values(
                \array_unique(\array_intersect(collect($record1), collect($record2)), \SORT_REGULAR)
            )
    )(...$args);
}
...
function intersectionWith(...$args)
{
    ...
    return curryN($arity, 
        fn (callable $comparator, ?iterable $record1, ?iterable $record2) => 
            values(
                \array_unique(\array_uintersect(collect($record1), collect($record2), $comparator), \SORT_REGULAR)
            )
    )(...$args);
}
